import { t, toast, confirmDialog } from './compat.js';
import {
  listSuppliers,
  createSupplier,
  updateSupplier,
  ...
  addSupplierPayment
} from '../queries/suppliers.js';
import { listPurchases, getPurchaseDetails, cancelPurchase } from '../queries/purchases.js';
  ...
  function draw() {
    ...
    container.querySelector('#supp-tbody').innerHTML = suppliers
      ...
      <tr>
        ...
        <td class="mono-num">
          ${balance > 0 ? `<span class="badge badge-warning">${balance.toFixed(2)}</span>` : `<span class="badge badge-success">${t('no_balance_due')}</span>`}
        </td>
        <td>
  ...
  function openPaymentModal(supplier) {
    ...
    overlay.innerHTML = `
      <div class="modal-box">
        ...
        <div class="modal-body">
          <div class="text-muted" style="margin-bottom:14px;">${t('supplier_balance')}: <span class="mono-num">${Number(supplier.balance).toFixed(2)}</span></div>
          <form id="pay-form">
            <div class="field">
              <label>${t('payment_amount')}</label>
              <input class="input" type="number" step="0.01" min="0.01" name="amount" required value="${Math.max(Number(supplier.balance), 0).toFixed(2)}" />
            </div>
            <div class="field">
      ...
      <div class="modal-box modal-lg">
        ...
        <div class="modal-body">
          <div class="card card-pad" style="background:var(--color-surface-2); margin-bottom:14px;">
            <div class="summary-row total-row"><span>${t('supplier_balance')}</span><span class="mono-num">${Number(supplier.balance).toFixed(2)}</span></div>
          </div>
          <div class="table-wrap">
            <table class="data-table">
